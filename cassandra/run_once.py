"""Drive ONE full end-to-end supervision cycle.

Moved into the package (from `scripts/run_pipeline.py`) so the pip-installed `cassandra run`
command works — `scripts/` is not shipped in the wheel. The raw httpx POST seeds a
deliberately production-looking span (no `session_id="test"`, so the Watcher does not filter
it as one of Cassandra's own probes). No `patient/` imports — the pipeline stays agent-agnostic.
"""
...
def main() -> None:
    """Sync entry point (console script / `cassandra run`)."""
