...
class RedTeam:
    ...
    async def _ask(self, client: httpx.AsyncClient, msg: str, override: str | None) -> dict:
        """Full agent response (reply + tool ledger) so the oracle can ground-score it."""
        return await ask_patient(client, msg, system_override=override)
    ...
    async def attack(self, inc: Incident) -> Incident:
        ...
        async with httpx.AsyncClient(timeout=300) as c:
            for ex in probes:
                before_out = await self._ask(c, ex.input_text, None)
                after_out = await self._ask(c, ex.input_text, inc.candidate_prompt)
                b, a = await asyncio.gather(
                    score_case(
                        ex.input_text,
                        ex.expected_answer,
                        before_out.get("reply", ""),
                        before_out.get("tool_calls"),
                    ),
                    score_case(
                        ex.input_text,
                        ex.expected_answer,
                        after_out.get("reply", ""),
                        after_out.get("tool_calls"),
                    ),
                )
